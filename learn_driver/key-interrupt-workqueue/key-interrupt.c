...
struct work_struct key_work_queue;

// 下半部
static void key_interrupt_handle(struct work_struct *key_work)
{
	printk("Ringing!!!\n");
	return ;
}
...
static int key_temp_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&key_work_queue, key_interrupt_handle);
